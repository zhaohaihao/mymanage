...
<form name="defaultForm" action="${siteUrl}/datacenter/channelStatistics" method="post" >
	...
	<table class="table table-striped table-bordered table-hover" style="min-width:1600px;">
		...
		<thead>
			<tr>
				...
				<@seqTh page "defaultForm" "channel" >渠道商</@seqTh>
				...
				<@seqTh page "defaultForm" "payRate" >付费比</@seqTh>
...
<div class="widget-content autoTbody">
	...
	<#list page.items as item>
		<tr>
			...
			<td>${item.channel!}</td>
			<td>
				<#if item.channel?? && item.channel == "ios">
					<@parseNum item.payAmountIos/>
				<#else>
					<@parseNum item.payAmount/>
				</#if>
			</td>
			<td>${item.payPl!}</td>
			<td>
...
<script type="text/javascript">
	function exportExcel(){
		document.defaultForm.action="${siteUrl}/datacenter/makeChannelExcel"; 
		defaultForm.submit();
	}
	
	function commitTable(){
		document.defaultForm.action="${siteUrl}/datacenter/channelStatistics"; 
		defaultForm.submit();
	}
</script>